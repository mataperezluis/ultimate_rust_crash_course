...
fn main() {
    ...
    let tx3_c = tx_c.clone();
    let rx3_c = rx_c.clone();

    // Cloning a channel makes another variable connected to that end of the channel so that you can
    // send it to another thread.

    let handle_ac = thread::spawn(move || {
        for msg in rx_c {
    ...
    let handle_bc = thread::spawn(move || {
        for msg in rx3_c {
    ...
    // closed.  Both our threads automatically close their Sender channels when they exit and the
    // destructors for the channels get automatically called.
    tx_c.send("epa A").unwrap();
    tx3_c.send("epa B").unwrap();
    drop(tx_c);
    drop(tx3_c);

    // Join the child threads for good hygiene.
    handle_ac.join().unwrap();
    //handle_bc.join().unwrap();
